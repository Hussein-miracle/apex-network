...
let isLoadingOverdueTable = ref<boolean>(false)
const overdueTableData = ref<Payment[]>([])
let page = ref<number>(1)
let perPage = ref<number>(6)
let total = ref<number>(0)

const fetchOverdueTableData = async (page: number = 1, perPage: number = 6) => {
  isLoadingOverdueTable.value = true
  try {
    const data = await paymentsService.GetPayments({ page, perPage, state: 'overdue' })
    console.log({ overdueTableDataLog: data })
    overdueTableData.value = formatPaymentTableData((data.data as unknown as Payment[]) ?? [])
    console.log({ overdueTableData: overdueTableData.value })
  } catch (err) {
    console.error(err)
  } finally {
    isLoadingOverdueTable.value = false
  }
}
...
const markItemSelected = (item: Payment) => {
  ...
  // console.log({ item })
  const updatedTableData = overdueTableData.value.map((tableItem) => {
    if (tableItem.id === updatedItem.id) {
      return updatedItem
    }
    return tableItem
  })
  overdueTableData.value = updatedTableData
}
...
<template>
  <section class="bg-white w-full rounded-2xl mt-8 flex flex-col justify-between">
      ...
      <div class="overflow-auto">
        <div class="inline-block min-w-full py-2 align-middle">
          <table class="min-w-full divide-y divide-gray-300">
            ...
            <TableSkeleton v-if="isLoadingOverdueTable" />
            <tbody
              class="divide-y divide-apex-grey-2"
              v-else-if="!isLoadingOverdueTable && !!overdueTableData"
            >
              <template v-if="overdueTableData.length > 0">
                <tr
                  v-for="transaction in overdueTableData"
                  :key="transaction.id"
    ...
    <main
      ...
      v-if="!isLoadingOverdueTable && overdueTableData.length > 0"
    >
      <Pagination />
    </main>
  </section>
