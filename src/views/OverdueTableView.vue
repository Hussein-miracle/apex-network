...
const fetchTableData = async (page: number = 1, per_page: number = 6, state: State = 'overdue') => {
  ...
  try {
    ...
    // console.log({ tableDataLogOverdue: tableDataCopy.value })
  } catch (err: any) {
    console.error(err)
    errorToast(err?.message ?? 'An error occurred while fetching overdue payments data')
  } finally {
