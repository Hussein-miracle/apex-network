...
let isLoadingUnpaidTable = ref<boolean>(false)
const unpaidTableData = ref<Payment[]>([])
let page = ref<number>(1)
let perPage = ref<number>(6)
let total = ref<number>(0)

const fetchUnpaidTableData = async (page: number = 1, perPage: number = 6) => {
  isLoadingUnpaidTable.value = true
  try {
    const data = await paymentsService.GetPayments({ page, perPage, state: 'unpaid' })
    console.log({ unpaidTableDataLog: data })
    unpaidTableData.value = formatPaymentTableData((data.data as unknown as Payment[]) ?? [])
    console.log({ unpaidTableData: unpaidTableData.value })
  } catch (err) {
    console.error(err)
  } finally {
    isLoadingUnpaidTable.value = false
  }
}
...
const markItemSelected = (item: Payment) => {
  ...
  // console.log({ item })
  const updatedTableData = unpaidTableData.value.map((tableItem) => {
    if (tableItem.id === updatedItem.id) {
      return updatedItem
    }
    return tableItem
  })
  unpaidTableData.value = updatedTableData
}

onMounted(() => {
  fetchUnpaidTableData()
})
</script>
...
<template>
  <section class="bg-white w-full rounded-2xl mt-8 flex flex-col justify-between">
      ...
      <div class="overflow-auto">
        <div class="inline-block min-w-full py-2 align-middle">
          <table class="min-w-full divide-y divide-gray-300">
            ...
            <TableSkeleton v-if="isLoadingUnpaidTable" />
            <tbody
              class="divide-y divide-apex-grey-2"
              v-else-if="!isLoadingUnpaidTable && !!unpaidTableData"
            >
              <template v-if="unpaidTableData.length > 0">
                <tr
                  v-for="transaction in unpaidTableData"
                  :key="transaction.id"
    ...
    <main
      ...
      v-if="!isLoadingUnpaidTable && unpaidTableData.length > 0"
    >
      <Pagination />
    </main>
  </section>
